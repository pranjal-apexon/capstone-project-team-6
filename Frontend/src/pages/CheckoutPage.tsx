...
const CheckoutPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Simulate frontend processing network delay (2 seconds)
        await new Promise((resolve) => setTimeout(resolve, 2000));

        setIsProcessing(false);
        setIsSuccess(true);

        // Clear the cart globally upon successful checkout simulation
        dispatch(clearCart());
    };
    ...
    if (isSuccess) {
        return (
            <div className="cart-empty success-container">
                ...
                <p>Thank you for your purchase. Your mock order has been processed on the frontend.</p>
                <button className="btn-filter-apply" onClick={() => navigate('/')}>
                    Continue Shopping
                </button>
            </div>
        );
    ...
    return (
        <div className="page-container checkout-page-layout">
            ...
            <div className="checkout-grid">
                ...
                <form onSubmit={handleSubmit} className="product-search-panel checkout-form">
                    ...
                    <div className="filter-group">
                        ...
                        <input type="text" name="cardNumber" required value={formData.cardNumber} onChange={handleInputChange} placeholder="0000 0000 0000 0000" maxLength={19} />
                    </div>

                    <div className="form-row" style={{ marginTop: '12px' }}>
                        <div className="filter-group">
                            <label>Expiry Date</label>
                            <input type="text" name="expiry" required value={formData.expiry} onChange={handleInputChange} placeholder="MM/YY" maxLength={5} />
                        </div>
                        <div className="filter-group">
                            <label>CVV</label>
                            <input type="password" name="cvv" required value={formData.cvv} onChange={handleInputChange} placeholder="123" maxLength={3} />
                        </div>
                    </div>

                    <button type="submit" className="btn-checkout" style={{ marginTop: '30px' }} disabled={isProcessing}>
                        {isProcessing ? 'Processing Payment...' : `Pay $ ${(totalAmount * 1.1).toFixed(2)}`}
                    </button>
                </form>
                ...
                <div className="cart-summary checkout-summary-box">
                    ...
                    <div className="checkout-items-list" style={{ marginBottom: '20px', maxHeight: '200px', overflowY: 'auto' }}>
                        {items.map((item) => (
                            <div key={item.productId} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', padding: '8px 0', borderBottom: '1px solid #eee' }}>
